...
<header data-header class="group fixed inset-x-0 top-0 z-50 transition-all duration-300">
  <div class="absolute inset-0 -z-10 bg-transparent backdrop-blur-0 transition-all duration-300 group-[.is-scrolled]:bg-white/95 group-[.is-scrolled]:backdrop-blur-md group-[.is-scrolled]:shadow-[0_1px_0_0_rgba(10,26,68,0.08)]"></div>
  <div class="container">
    <div class="flex h-20 items-center justify-between sm:h-24">
      <a href="index.html" class="flex items-center gap-2.5 shrink-0" aria-label="FIRST-COM GROUP — Accueil">
        <img src="images/logo/logo-color.webp" alt="FIRST-COM GROUP" class="h-9 sm:h-10 w-auto brightness-0 invert transition-all duration-300 group-[.is-scrolled]:brightness-100 group-[.is-scrolled]:invert-0" />
      </a>
      ...
      <div class="hidden lg:flex items-center gap-3">
        <a href="tel:{phone_tel}" class="hidden xl:inline-flex items-center gap-2 text-sm font-semibold text-white/90 transition-colors hover:text-brick-500 group-[.is-scrolled]:text-navy-800">
          {phone_icon}
          {phone_display}
        </a>
      ...
      <button type="button" data-menu-toggle aria-expanded="false" aria-controls="mobile-menu" aria-label="Ouvrir le menu"
        class="lg:hidden inline-flex h-11 w-11 items-center justify-center rounded-full text-white ring-1 ring-white/40 transition-colors group-[.is-scrolled]:text-navy-900 group-[.is-scrolled]:ring-ink-200">
  ...
  <div id="mobile-menu" data-mobile-menu class="hidden lg:hidden fixed inset-0 top-20 sm:top-24 z-40 overflow-y-auto bg-white px-6 pb-10 pt-2">
...
def home_hero():
    return """
<section class="relative overflow-hidden bg-navy-gradient pb-24 pt-40 sm:pb-32 sm:pt-48">
  <div class="absolute inset-0 bg-dots-dark opacity-60"></div>
  <div class="pointer-events-none absolute -left-40 top-24 h-96 w-96 rounded-full bg-navy-600/30 blur-3xl"></div>
  <div class="pointer-events-none absolute right-0 top-0 h-[32rem] w-[32rem] rounded-full bg-brick-600/10 blur-3xl"></div>
  <div class="fiber-line absolute left-0 right-0 top-20 opacity-40"></div>

  <div class="container relative grid grid-cols-1 items-center gap-16 lg:grid-cols-12 lg:gap-8">
    <div class="lg:col-span-7">
      <p data-hero-eyebrow class="eyebrow !text-brick-400">
        <span class="h-[2px] w-6 bg-brick-500 block"></span>
        FIRST-COM GROUP — Attécoubé, Abidjan
      </p>
      <h1 data-hero-title class="mt-6 text-display-xl font-display font-bold text-white text-balance">
        """ + split_words("La technologie qui connecte,") + """<br class="hidden sm:block" />
        """ + split_words("sécurise et fait avancer") + """<br class="hidden sm:block" />
        <span class="text-brick-500">""" + split_words("votre entreprise.") + """</span>
      </h1>
      <p data-hero-sub class="mt-7 max-w-xl text-lg leading-relaxed text-white/70">
      ...
      <div data-hero-stats class="mt-14 grid max-w-lg grid-cols-3 gap-6 border-t border-white/10 pt-8">
        <div>
          <p class="stat-num"><span data-count="8">0</span></p>
          <p class="mt-1 text-xs font-medium uppercase tracking-wide text-white/50">Domaines d'expertise</p>
        </div>
        <div>
          <p class="stat-num"><span data-count="19">0</span>+</p>
          <p class="mt-1 text-xs font-medium uppercase tracking-wide text-white/50">Organisations qui nous font confiance</p>
        </div>
        <div>
          <p class="stat-num"><span data-count="10">0</span></p>
          <p class="mt-1 text-xs font-medium uppercase tracking-wide text-white/50">Partenaires technologiques</p>
        </div>
      </div>
    </div>
    ...
    <div class="lg:col-span-5">
      <div data-hero-visual class="relative mx-auto max-w-md lg:max-w-none">
        <div data-hero-parallax class="relative">
          <div class="relative overflow-hidden rounded-[1.75rem] shadow-glow ring-1 ring-white/10">
          ...
          <div data-float-card class="absolute -left-6 -bottom-8 w-52 rounded-2xl bg-white/95 p-4 shadow-glow ring-1 ring-white/40 backdrop-blur sm:-left-10">
          ...
          <div data-float-card class="absolute -right-4 -top-6 flex items-center gap-2 rounded-full bg-white/95 px-4 py-2.5 shadow-glow ring-1 ring-white/40 backdrop-blur sm:-right-8">
    ...
    for i, (num, title, desc) in enumerate(APPROACH):
        ...
        <div data-reveal class="flex flex-1 flex-col items-start lg:items-center lg:text-center">
          <div class="flex items-center gap-3 lg:flex-col lg:gap-4">
            ...
            <span class="hidden h-10 w-px bg-white/15 lg:hidden"></span>
          </div>
          <h3 class="mt-3 font-display text-base font-semibold text-white lg:mt-2">{title}</h3>
          <p class="mt-2 max-w-[15rem] text-sm leading-relaxed text-white/60">{desc}</p>
        </div>
        {connector}
        """.format(num=num, title=title, desc=desc, connector=connector if not last else ""))
    steps_html = "\n".join(steps)
    return """
<section class="section bg-navy-950 relative overflow-hidden">
  <div class="absolute inset-0 bg-dots-dark opacity-30"></div>
  <div class="container relative">
    <div class="mx-auto max-w-2xl text-center">
      <p data-reveal class="eyebrow justify-center !text-brick-400">Notre approche</p>
      <h2 data-reveal class="mt-4 text-display-md font-bold text-white text-balance">
...
def page_banner(eyebrow, title, subtitle, extra=""):
    return """
<section class="relative overflow-hidden bg-navy-gradient pb-20 pt-36 sm:pb-24 sm:pt-44">
  <div class="absolute inset-0 bg-dots-dark opacity-50"></div>
  <div class="pointer-events-none absolute -right-32 -top-16 h-96 w-96 rounded-full bg-navy-600/25 blur-3xl"></div>
  <div class="container relative">
    <p data-reveal class="eyebrow !text-brick-400">
      <span class="h-[2px] w-6 bg-brick-500 block"></span>
      {eyebrow}
    </p>
    <h1 data-reveal class="mt-5 max-w-3xl text-display-lg font-bold text-white text-balance">{title}</h1>
    <p data-reveal class="mt-5 max-w-2xl text-lg leading-relaxed text-white/70">{subtitle}</p>
    {extra}
  </div>
</section>
...
def build_sitemap():
    pages = ["index.html", "a-propos.html", "services.html", "realisations.html", "contact.html", "mentions-legales.html", "confidentialite.html"]
    urls = "\n".join(
        """  <url>
    <loc>{site}/{p}</loc>
